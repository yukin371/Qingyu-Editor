...
test('正文内创建 @实体 后会进入本章设定投影', async ({ page }) => {
  ...
  const rightPanel = page.getByTestId('resizable-panel-right')
  await rightPanel.getByRole('button', { name: '本章' }).click()
  ...
  await expect(rightPanel.getByText('沈砚')).toBeVisible()
})
...
test('删除正文里的 @实体 只解除本章引用，不删除全局资产', async ({ page }) => {
  ...
  await expect(rightPanel.getByText('沈砚')).toBeVisible()
  await rightPanel.getByRole('button', { name: '全局' }).click()
  await expect(rightPanel.getByText('沈砚')).toBeVisible()

  await rightPanel.getByRole('button', { name: '本章' }).click()
  await clearEditorBody(page)
  await expectSaveStatus(page, '已保存')
  await expect(rightPanel.getByText('沈砚')).not.toBeVisible()
