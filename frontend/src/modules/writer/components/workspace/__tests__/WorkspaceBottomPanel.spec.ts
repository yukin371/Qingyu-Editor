...
describe('WorkspaceBottomPanel', () => {
  it('renders the resizable scene stage with persisted height', () => {
    ...
    expect(wrapper.get('.workspace-bottom-panel__status').text()).toContain('雨夜祠堂')
    expect(wrapper.get('.workspace-bottom-panel__status').text()).toContain('当前章节：第一章')
    expect(wrapper.get('.workspace-bottom-panel__status').text()).toContain('覆盖：第一章')
