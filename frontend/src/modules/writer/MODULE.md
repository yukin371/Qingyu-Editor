...
- **右栏校对默认走系统规则，AI 只做深度审校**：`ProofreadPanel -> useProofreadPanel` 是 Wails 本地编辑器校对入口，默认“系统校对”只跑本地规则，不调用 AI，以控制成本并支持离线基础检查；用户显式点击“AI 深度审校”时才消费 `modules/ai/api/workbench.proofreadContent`。AI 不可用时保留系统规则结果。校对结果只作为建议展示，不静默改正文；正文变化后旧 issue 标记为过期，避免旧位置继续误导作者。
- **错词表必须走词库适配层**：常见错词、成语错写等词表由 `services/proofreadLexicon.service.ts` 读取 `data/proofreadLexicons/*.json`，校对规则只消费统一 `ProofreadLexicon` schema。默认外部成语词库使用 MIT 许可的 `cnchar / cnchar-idiom`，只作为本地系统规则生成高置信成语错写建议；后续接其他开源词库、后端导出词库或用户自定义词库时，应转换为同一 schema，不要继续把词条硬编码进 `useProofreadPanel.ts`。用户白名单、自定义词条和 JSON 导入/导出归该 service，优先 `localStorage` 持久化，无 storage 时允许内存级 fallback；右栏词库弹窗只做轻量管理和 schema 级导入/导出，批量外部词库导入仍需先确认许可证和 schema 适配。
- **项目资产白名单由宿主传入，不反向依赖资产 API**：角色、角色别名、地点和当前章节活跃资产会由 `ProjectWorkspace -> WorkspaceRightPanel -> ToolRightPanel -> ProofreadPanel -> useProofreadPanel` 作为 `ignoredTerms` 传入系统校对，用来降低专名误报；`proofreadLexicon.service.ts` 不得主动拉取 `character / location / entities / concept` API，避免词库 owner 反向绑定资产 owner。后续若补全物件、组织、概念的全项目白名单，应仍由宿主或资产聚合层提供词项。
- **校对高亮只读挂在正文编辑器**：Wails 校对问题通过 `ProofreadPanel -> ToolRightPanel -> WorkspaceRightPanel -> ProjectWorkspace -> WorkspaceEditorContent -> TipTapEditorView -> QyTipTapEditor` 传递为 `ProofreadHighlightRange`，再由 `ProofreadHighlightExtension` 渲染 Tiptap Decoration。高亮与定位不写入正文 JSON，不接管保存链路；`originalText` 不匹配或 issue 过期时不得继续定位。
- **校对建议应用必须进入正文 diff**：右栏只对带 `replacementText` 的结构化替换建议显示“应用”，包括 AI issue、本地内置词库、自定义词条和外部成语库命中；点击后由 `ProjectWorkspace` 校验 `position + originalText`，再注册 `AiDiffExtension` pending diff，作者仍需在正文里接受或放弃。解释性本地规则建议不得被当作替换文本自动应用，展示文案只放在 `suggestion`。
...
- **底栏只做可编辑场景舞台**：底部区域现在只承接当前场景、在场资产和当前拍管理；状态栏保留显性的“场景舞台”入口，状态 / 上下文 / 审查等旧底栏内容不得再作为默认面板扩张，深度审查和资产整理继续放在右侧工具或 overlay。底栏高度由 `workspaceLayoutStore.bottomPanel.height` 持久化，拖拽把手只调整场景舞台高度，不引入新的底栏工具宿主。场景舞台默认展示当前章节、系统覆盖、纳入状态和动作按钮，核心编辑字段常驻三栏区域，不要求作者先点“编辑”；三栏是结构分区，外层不滚动，每列自己滚动，不用卡片套卡片；窄屏下也保持三栏并用横向滚动承载，不能折回纵向长列表。场景名只有作者手动填写才成立，章节名只能出现在“当前章节 / 系统覆盖”里，不得被当成默认场景名。当前拍可跨越多个章节，覆盖范围只能从当前章节向前选择连续倒数窗口（如 `第三章`、`第二章 - 第三章`），不得由切章历史累积出断档范围。字段包括当前拍、覆盖章节、状态、目标、冲突、完成条件和下一拍预告；只有用户点击“进入下一拍”才推进节拍，点击“新场景”才切换叙事段，不按章节自动切换。节拍草稿由 `useWriterSceneStage` 作为项目级本地 sidecar 保存，不写正文、不接管结构舞台。
- **顶部右侧工具按钮是打开入口，不做隐藏开关**：`WorkspaceTopbar` 的 `设定 / 校对 / 灵感 / AI 助手` 只负责把对应右侧工具设为 active 并打开右栏；移动端还必须切到右侧 pane。隐藏右栏应使用右栏内关闭按钮或右侧 activity bar 的折叠行为，避免用户点“设定”却只改变了不可见状态。
